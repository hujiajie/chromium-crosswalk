...
{
    std::unique_ptr<WebCLEventHolder> holder(static_cast<WebCLEventHolder*>(userData));
    holder->event2 = event;
    holder->type2 = type;

    if (!isMainThread()) {
        Platform::current()->mainThread()->getWebTaskRunner()->postTask(
            BLINK_FROM_HERE, threadSafeBind(&WebCLEvent::callbackProxyOnMainThread, passed(std::move(holder))));
        return;
    }
...
void WebCLEvent::callbackProxyOnMainThread(std::unique_ptr<WebCLEventHolder> holder)
{
    ASSERT(isMainThread());
