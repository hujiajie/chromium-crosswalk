...
{
    std::unique_ptr<WebCLProgramHolder> holder(static_cast<WebCLProgramHolder*>(userData));
    ...
    Platform::current()->mainThread()->getWebTaskRunner()->postTask(BLINK_FROM_HERE, threadSafeBind(&WebCLProgram::callbackProxyOnMainThread, passed(std::move(holder))));
}

void WebCLProgram::callbackProxyOnMainThread(std::unique_ptr<WebCLProgramHolder> holder)
{
    ASSERT(isMainThread());
