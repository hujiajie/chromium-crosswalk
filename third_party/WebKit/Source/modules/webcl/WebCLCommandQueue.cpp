...
{
    std::unique_ptr<WebCLCommandQueueHolder> holder(static_cast<WebCLCommandQueueHolder*>(userData));
    holder->event = event;
    holder->type = type;

    if (!isMainThread()) {
        Platform::current()->mainThread()->getWebTaskRunner()->postTask(BLINK_FROM_HERE, threadSafeBind(&WebCLCommandQueue::callbackProxyOnMainThread, passed(std::move(holder))));
        return;
    }

    callbackProxyOnMainThread(std::move(holder));
}

void WebCLCommandQueue::callbackProxyOnMainThread(std::unique_ptr<WebCLCommandQueueHolder> holder)
{
    ASSERT(isMainThread());
